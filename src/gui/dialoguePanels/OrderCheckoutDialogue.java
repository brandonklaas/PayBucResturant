 ...
 * @author brand
 */
public class OrderCheckoutDialogue extends javax.swing.JPanel {
...
//    private ComboBoxModel drinksModel;
     ...
     */
    public OrderCheckoutDialogue(Session session) {
        this.session = session;
        initComponents();
        setDefaults();
    }

    public OrderCheckoutDialogue( ModernUI desktop, Order order, Session session) { 
        this.session = session;
    ...
    private void saveBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveBtnActionPerformed
        ...
        if(products.size() > 0 && waiterCB.getSelectedIndex() > 0){
            ...
            if(database.update(currentOrder)){
                new OkayDialogue(desktop, true, "Order updated successfully");
                diag.dispose();
            } else {
                new OkayDialogue(desktop, true, "Order not updated");
            }
        }
    ...
    private void servedBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_servedBtnActionPerformed
        // TODO add your handling code here:
        if(preCartTable.getSelectedRow() > -1){
            products.get(preCartTable.getSelectedRow()).setProductStatus(ProductStatus.SERVED.getID());
            clearPanel();
            updateTable();
        } else {
            new OkayDialogue(desktop, true, "Select product");
        }
    }//GEN-LAST:event_servedBtnActionPerformed
    ...
    private javax.swing.JButton servedBtn;
